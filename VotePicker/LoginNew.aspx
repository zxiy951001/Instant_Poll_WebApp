<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LoginNew.aspx.cs" Inherits="votepicker_webproj.LoginNew" EnableEventValidation="false" MaintainScrollPositionOnPostBack="true" %>


<!DOCTYPE html>
<html lang="es" dir="ltr">
 
     
<head>
    <meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0">
    <meta charset="utf-8">
    <link rel="stylesheet" href="css/mainlogin.css">
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;700;800&display=swap" rel="stylesheet">
    <script type="text/javaScript"
        src="http://ajax.googleapis.com/ajax/libs/jquery/1.7.2/jquery.min.js"></script>
     <script type="text/javaScript">
         $(document).ready(function () {
             //Show_data();
             $("#userlogin").click(function () {
                 var username = $("#txtusername").val();
                 var password= $("#txtpassword").val();

                 if (username.value == "" || password.value == "") {
                     alert("EmptyInput")
                     return false;
                 }

                 $.ajax({
                     type: 'POST',
                     url: '<%=ResolveUrl("LoginNew.aspx/saveData")%>',
                     dataType: 'json',
                     contentType: "application/json; charset=utf-8",
                     data: JSON.stringify({
                            username: $('#txtusername').val(),
                            password: $('#txtpassword').val(),
                        }),
                    
                     success: function (result) {

                         successfulstringifiedjson = { "d": "presenter" };
                         successfulstringifiedjson1 = { "d": "suspended" };
                         successfulstringifiedjson2 = { "d": "invalid" };
                         successfulstringifiedjson3 = { "d": "admin" };
                         if (Object.keys(successfulstringifiedjson).length == Object.keys(result).length) {
                             for (key in successfulstringifiedjson) {
                                 if (successfulstringifiedjson[key] == result[key]) {
                                     var username = $("#txtusername").val();
                                     '<%Session["username"] =  "Presenter"; %>'; 
                                                                     
                                     window.location.href = "Default.aspx";
                                 }
                             }
                         }
                         if (Object.keys(successfulstringifiedjson1).length == Object.keys(result).length) {
                             for (key in successfulstringifiedjson1) {
                                 if (successfulstringifiedjson1[key] == result[key]) {
                                     document.getElementById("errtext").innerHTML = "Your Account Has Been Suspended!";
                                 }
                             }

                         }


                         if (Object.keys(successfulstringifiedjson2).length == Object.keys(result).length) {
                             for (key in successfulstringifiedjson2) {
                                 if (successfulstringifiedjson2[key] == result[key]) {
                                     document.getElementById("errtext").innerHTML = "Invalid Login Credentials!";
                                 }
                             }

                         }

                         if (Object.keys(successfulstringifiedjson3).length == Object.keys(result).length) {
                             for (key in successfulstringifiedjson3) {
                                 if (successfulstringifiedjson3[key] == result[key]) {
                                    '<%Session["username"] =  "Admin"; %>'; 
                                     window.location.href = "admin.aspx";
                                 }
                             }

                         }

                     }
                  
                 });
             });
         });

      

              
             

       
           </script> 
</head>

<body>
       
    
    <div class="main">
        <div class="container a-container" id="a-container">
            <form class="form" id="aform"  runat="server">
                <h2 class="form_title title">Create Account</h2>
                <div class="form__icons"><img class="form__icon" src="data:image/svg+xml;base64,"
                        alt=""><img class="form__icon" src="data:image/svg+xml;base64,">
                    <img
                        class="form__icon" src="data:image/svg+xml;base64,"></div><span class="form__span">or use email for registration</span><asp:TextBox class="form__input" id="name" type="text" placeholder="Name" runat="server"/><asp:TextBox class="form__input" id="Username" runat="server" type="text" placeholder="Username"/><asp:TextBox class="form__input" type="text" placeholder="Email" id="Email" runat="server"/><asp:TextBox class="form__input" type="password" placeholder="Password" id="Password" runat="server"/>
                <button
                    class="form__button button submit" runat="server" onserverclick="BtnRegister_Click">SIGN UP</button>
                <a class="txt5" runat="server" id="readerrortext"/>	
            </form>
        </div>
        <div class="container b-container" id="b-container">
            <form class="form" id="bform" method="" action="" ">
                <h2 class="form_title title">Sign in to Website</h2>
                <div class="form__icons"><img class="form__icon" src="data:image/svg+xml;base64,"
                        alt=""><img class="form__icon" src="data:image/svg+xml;base64,">
                    <img
                        class="form__icon" src="data:image/svg+xml;base64,"></div><span class="form__span">or use your email account</span><input class="form__input" id="txtusername" type="text" placeholder="username"><input class="form__input" id="txtpassword" type="password" placeholder="Password"><a class="form__link">Forgot your password?</a>

               
                
      
                <button class="form__button button submit" id="userlogin" value="Submit">
                    SIGN IN</button>


                <p></p>
                <p></p>
                <a class="txt5"  id="errtext"/>	

              
                    

            </form>
        </div>
        <div class="switch" id="switch-cnt">
            <div class="switch__circle"></div>
            <div class="switch__circle switch__circle--t"></div>
            <div class="switch__container" id="switch-c1">
                <h2 class="switch__title title">Welcome Back !</h2>
                <p class="switch__description description">To keep connected with us please login with your personal info</p><button class="switch__button button switch-btn">SIGN IN</button></div>
            <div class="switch__container is-hidden" id="switch-c2">
                <h2 class="switch__title title">Hello Friend !</h2>
                <p class="switch__description description">Enter your personal details and start journey with us</p><button class="switch__button button switch-btn">SIGN UP</button></div>
        </div>
    </div>
  
    <script src="css/main.js"></script>
     
   
   
   
</body>


</html>